...
public class PcapReader implements Iterable<Packet> {
	...
	public static final int PCAP_HEADER_TIMESTAMP_OFFSET = 8;
	...
	public static final int TOTAL_SIZE_OFFSET = 12;
	...
	public static final int SLL_HEADER_BASE_SIZE = 10; // SLL stands for Linux
														// cooked-mode capture
	public static final int SLL_ADDRESS_LENGTH_OFFSET = 4; // relative to SLL
															// header
	...
	public static final int IP_TOTAL_LEN_OFFSET = 2; // relative to start of IP
														// header
	public static final int IPV6_PAYLOAD_LEN_OFFSET = 4; // relative to start of
															// IP header
	public static final int IPV6_HOPLIMIT_OFFSET = 7; // relative to start of IP
														// header
	public static final int IP_PROTOCOL_OFFSET = 9; // relative to start of IP
													// header
	public static final int IPV6_NEXTHEADER_OFFSET = 6; // relative to start of
														// IP header
	public static final int IP_SRC_OFFSET = 12; // relative to start of IP
												// header
	public static final int IPV6_SRC_OFFSET = 8; // relative to start of IP
													// header
	public static final int IP_DST_OFFSET = 16; // relative to start of IP
												// header
	public static final int IPV6_DST_OFFSET = 24; // relative to start of IP
													// header
	...
	public PcapReader(DataInputStream is) throws IOException {
		...
		if (!validateMagicNumber(pcapHeader))
			throw new IOException(
					"Not a PCAP file (Couldn't find magic number)");

		snapLen = PcapReaderUtil.convertInt(pcapHeader,
				PCAP_HEADER_SNAPLEN_OFFSET, reverseHeaderByteOrder);

		TimeStampNew = PcapReaderUtil.convertInt(pcapHeader,
				PCAP_HEADER_TIMESTAMP_OFFSET, reverseHeaderByteOrder);

		long linkTypeVal = PcapReaderUtil.convertInt(pcapHeader,
				PCAP_HEADER_LINKTYPE_OFFSET, reverseHeaderByteOrder);
	...
	private int getUdpLength(byte[] packetData, int ipStart, int ipHeaderLen) {
		int udpLen = PcapReaderUtil.convertShort(packetData, ipStart
				+ ipHeaderLen + 4);
		return udpLen;
	}
	...
	private Packet nextPacket() {
		...
		long packetTimestamp = PcapReaderUtil.convertInt(pcapPacketHeader,
				TIMESTAMP_OFFSET, reverseHeaderByteOrder);
		packet.put(Packet.TIMESTAMP, packetTimestamp);

		long packetTimestampMicros = PcapReaderUtil.convertInt(
				pcapPacketHeader, TIMESTAMP_MICROS_OFFSET,
				reverseHeaderByteOrder);
		packet.put(Packet.TIMESTAMP_MICROS, packetTimestampMicros);

		long totalSize = PcapReaderUtil.convertInt(pcapPacketHeader,
				TOTAL_SIZE_OFFSET, reverseHeaderByteOrder);
		...
		long packetSize = 0L;
		int value = 0;
		packetSize = PcapReaderUtil.convertInt(pcapPacketHeader,
				CAP_LEN_OFFSET, reverseHeaderByteOrder);

		value = (int) packetSize;
//		int positive = Math.abs(value);
		...
		int ipProtocolHeaderVersion = getInternetProtocolHeaderVersion(
				packetData, ipStart);
		packet.put(Packet.IP_VERSION, ipProtocolHeaderVersion);

		if (ipProtocolHeaderVersion == 4 || ipProtocolHeaderVersion == 6) {
			int ipHeaderLen = getInternetProtocolHeaderLength(packetData,
					ipProtocolHeaderVersion, ipStart);
			...
			if (ipProtocolHeaderVersion == 4) {
				...
				totalLength = PcapReaderUtil.convertShort(packetData, ipStart
						+ IP_TOTAL_LEN_OFFSET);
			} else if (ipProtocolHeaderVersion == 6) {
				buildInternetProtocolV6Packet(packet, packetData, ipStart);
				int payloadLength = PcapReaderUtil.convertShort(packetData,
						ipStart + IPV6_PAYLOAD_LEN_OFFSET);
			...
			if (PROTOCOL_UDP == protocol || PROTOCOL_TCP == protocol) {
				...
				byte[] packetPayload = buildTcpAndUdpPacket(packet, packetData,
						ipProtocolHeaderVersion, ipStart, ipHeaderLen,
						totalLength);
				...
				if (isReassemble() && PROTOCOL_TCP == protocol) {
					...
					if (packetPayload.length > 0) {
						...
						SequencePayload sequencePayload = new SequencePayload(
								seq, packetPayload);
						flows.put(flow, sequencePayload);
					}

					if ((Boolean) packet.get(Packet.TCP_FLAG_FIN)
							|| (isPush() && (Boolean) packet
									.get(Packet.TCP_FLAG_PSH))) {
						Collection<SequencePayload> fragments = flows
								.removeAll(flow);
						if (fragments != null && fragments.size() > 0) {
							packet.put(Packet.REASSEMBLED_FRAGMENTS,
									fragments.size());
							...
							for (SequencePayload seqPayload : fragments) {
								if (prev != null && !seqPayload.linked(prev)) {
									LOG.warn("Broken sequence chain between "
											+ seqPayload + " and " + prev
											+ ". Returning empty payload.");
									packetPayload = new byte[0];
									break;
								}
								packetPayload = Bytes.concat(packetPayload,
										seqPayload.payload);
								prev = seqPayload;
							}
						}
	...
	protected int findIPStart(byte[] packet) {
		...
		case EN10MB:
			...
			int etherType = PcapReaderUtil.convertShort(packet,
					ETHERNET_TYPE_OFFSET);
			if (etherType == ETHERNET_TYPE_8021Q) {
				etherType = PcapReaderUtil.convertShort(packet,
						ETHERNET_TYPE_OFFSET + 4);
				start += 4;
			}
		...
		case LINUX_SLL:
			...
			int sllAddressLength = PcapReaderUtil.convertShort(packet,
	...
	private int getInternetProtocolHeaderLength(byte[] packet,
			int ipProtocolHeaderVersion, int ipStart) {
		if (ipProtocolHeaderVersion == 4)
	...
	private void buildInternetProtocolV4Packet(Packet packet,
			byte[] packetData, int ipStart) {
		int ttl = packetData[ipStart + IP_TTL_OFFSET] & 0xFF;
		packet.put(Packet.TTL, ttl);

		int protocol = packetData[ipStart + IP_PROTOCOL_OFFSET];
		packet.put(Packet.PROTOCOL,
				PcapReaderUtil.convertProtocolIdentifier(protocol));

		String src = PcapReaderUtil.convertAddress(packetData, ipStart
				+ IP_SRC_OFFSET, 4);
		packet.put(Packet.SRC, src);

		String dst = PcapReaderUtil.convertAddress(packetData, ipStart
				+ IP_DST_OFFSET, 4);
		packet.put(Packet.DST, dst);
	}

	private void buildInternetProtocolV6Packet(Packet packet,
			byte[] packetData, int ipStart) {
	...
	private byte[] buildTcpAndUdpPacket(Packet packet, byte[] packetData,
			int ipProtocolHeaderVersion, int ipStart, int ipHeaderLen,
			int totalLength) {
		packet.put(
				Packet.SRC_PORT,
				PcapReaderUtil.convertShort(packetData, ipStart + ipHeaderLen
		...
		if (PROTOCOL_UDP.equals(protocol)) {
			...
			tcpOrUdpHeaderSize = getTcpHeaderLength(packetData, ipStart
					+ ipHeaderLen);
			...
			packet.put(
					Packet.TCP_SEQ,
					PcapReaderUtil.convertUnsignedInt(packetData, ipStart
							+ ipHeaderLen + PROTOCOL_HEADER_TCP_SEQ_OFFSET));
			packet.put(
					Packet.TCP_ACK,
					PcapReaderUtil.convertUnsignedInt(packetData, ipStart
							+ ipHeaderLen + PROTOCOL_HEADER_TCP_ACK_OFFSET));
			...
			int flags = PcapReaderUtil.convertShort(new byte[] {
					...
					packetData[ipStart + ipHeaderLen + TCP_HEADER_DATA_OFFSET
							+ 1] }) & 0x1FF; // Filter first 7 bits. First 4 are
												// the data offset and the other
	...
	protected byte[] readPayload(byte[] packetData, int payloadDataStart,
		...
		if (payloadDataStart > packetData.length) {
			LOG.warn("Payload start (" + payloadDataStart
					+ ") is larger than packet data (" + packetData.length
					+ "). Returning empty payload.");
			return new byte[0];
		}
		if (payloadDataStart + payloadLength > packetData.length) {
			if (payloadDataStart + payloadLength <= snapLen) // Only corrupted
																// if it was not
																// because of a
																// reduced snap
																// length
				LOG.warn("Payload length field value ("
						+ payloadLength
						+ ") is larger than available packet data ("
	...
	private class PacketIterator implements Iterator<Packet> {
		...
		public boolean hasNext() {
			...
			if (remainingFlows > 0)
				LOG.warn("Still " + remainingFlows
						+ " flows queued. Missing packets to finish assembly?");
			return false;
		}
	...
	private class SequencePayload implements Comparable<SequencePayload> {
		...
		public String toString() {
			return Objects.toStringHelper(this.getClass()).add("seq", seq)
					.add("len", payload.length).toString();
		}
	}
}
